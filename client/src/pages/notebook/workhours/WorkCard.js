...
const WorkCard = ({
  ...
  const calcWeek = () => {
    ...
    // axios update
    axios
      .put('/api/workhours/' + workhourid, { weeklyHours: weekly })
      .then(res => {
  ...
  return (
    <div className='ui fluid card centered'>
        ...
        <div className='header'>
          <div className='right floated meta'>
            <button className='no-style-button'>
              <i
                className='trash light small icon'
                onClick={() => deleteWorkhours(workhourid)}
              ></i>
            </button>
          </div>
      ...
      <div className='content'>
        <strong
          className={
            workhour.weeklyHours > 45 ? 'negativeText' : 'positiveText'
          }
        >
          {workhour.weeklyHours} hours
        </strong>
